...
import { getCategoryTypeFromQuery, parseOrderByField } from '../../src/utils/common/serial'
...
describe('src/utils/common', () => {
  ...
  describe('test some series utils', () => {
    it('getCategoryTypeFromQuery', () => {
      ...
      expect(getCategoryTypeFromQuery(query)).toBe(CategoryType.ByGroup)

      query = Immutable({ outStatistics }) as IMFeatureLayerQueryParams

      expect(getCategoryTypeFromQuery(query)).toBe(CategoryType.ByField)
    })
  })
