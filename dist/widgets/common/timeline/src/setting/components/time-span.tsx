...
const TimeSpan = function (props: TimeSpanProps) {
  const { startTime, endTime, theme, intl, width = 200, dataSources, onChange } = props
      ...
      >
        {
          layersInfo?.map((layer, index) => {
            ...
            return <div className='d-flex align-items-center mb-2' key={'layerline-' + index}>
              <Tooltip placement='bottom'
                title={<div className='p-2'><div>{layer.label}</div><div>{`${layer.extentLabels[0]}-${layer.extentLabels[1]}`}</div></div>}
              >
                <Button icon size='sm' type='tertiary' className='p-0' >
        ...
        <div className='d-flex align-items-center'>
          <Tooltip title='Final extent' placement='bottom'>
            <Button icon size='sm' type='tertiary' className='p-0' >
          ...
          </Tooltip>
          {
            wholeExtent && <InputRange
              ...
              isRTL={isRTL}
              intl={intl}
              shadowHeight={layersInfo.length * 26 + 12}
