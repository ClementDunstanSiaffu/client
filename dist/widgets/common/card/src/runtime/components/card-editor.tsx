...
> {
  ...
  private readonly getCopyDropdownItems = (
    ...
    const isWidgetInLayout = (layoutId: string, widgetId: string): boolean => {
      const searchUtils = builderSupportModules.widgetModules.searchUtils
      const widgets = searchUtils.getContentsInLayoutWithLayoutWidgetOnly(
        appConfig,
        layoutId,
        LayoutItemType.Widget,
        browserSizeMode
  ...
  renderCardEditor = () => {
    ...
    const regularBgStyle = cardConfigs[Status.Regular].backgroundStyle.setIn(
      ['boxShadow', 'color'],
      'transparent'
    )
  ...
  render () {
    const { cardConfigs, widgetId, isEditing, builderStatus } = this.props
    ...
    return (
      ...
      >
        ...
        {!isEditing && <div className='edit-mask position-absolute' />}
      </div>
    )
  }
}
