/** @jsx jsx */
import { React, jsx, classNames, Immutable, IconResult, defaultMessages, IMState, IMAppConfig, LayoutInfo, LayoutType/*, getAppStore*/ } from 'jimu-core'
...
import { Button, Icon, Switch, TextInput, Select, Label, ButtonGroup, defaultMessages as commonMessages } from 'jimu-ui'
...
const UiModeStyleSize = {
  Popup: { w: 100, h: 100 },
  InlineHorizontal: { w: 578, h: 68 }, // Horizontal
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig> & ExtraProps> {
  ...
  render () {
    ...
    const popupDes = this.props.intl.formatMessage({ id: 'popupDes', defaultMessage: nls.popupDes })
    const inlineDes = this.props.intl.formatMessage({ id: 'inlineDes', defaultMessage: nls.inlineDes })
    ...
    if (uiMode === UiMode.Popup) {
      ...
      description = popupDes
    } else if (uiMode === UiMode.Inline) {
      subSettingUI = this.renderInlineModeSetting()
      description = inlineDes
    ...
    return (
      <div css={getStyle(this.props.theme)} className='widget-setting-menu jimu-widget-setting'>
        ...
        <SettingSection title={shareTypes}>
          <SettingRow>
            <div className='ui-mode-card-chooser'>
              <Label className='d-flex flex-column ui-mode-card-wapper'>
                <Button icon className={classNames('ui-mode-card', { active: (this.props.config.uiMode === UiMode.Popup) })}
                  onClick={() => this.onUIModeChanged(UiMode.Popup)}
                  aria-labelledby={a11yUIMode0Id} aria-describedby={a11yDescriptionId}>
                  <Icon width={100} height={80} icon={require('./assets/style0.svg')} autoFlip />
                </Button>
              ...
              <Label className='d-flex flex-column ui-mode-card-wapper'>
                <Button icon className={classNames('ui-mode-card', { active: (this.props.config.uiMode === UiMode.Inline) })}
                  onClick={() => this.onUIModeChanged(UiMode.Inline)}
                  aria-labelledby={a11yUIMode1Id} aria-describedby={a11yDescriptionId}>
                  <Icon width={100} height={80} icon={require('./assets/style1.svg')} autoFlip />
                </Button>
                <div id={a11yUIMode1Id} className='mx-1 text-break ui-mode-label'>{inline}</div>
              </Label>
              {/* <div className="ui-mode-card-wapper">
              <img className={'ui-mode-card' + this._activeUiModeStyle(UiMode.Slide)} id="uimode-2"
          ...
          <SettingRow>
            <div id={a11yDescriptionId} className='text-break'>
              {description}
            </div>
          </SettingRow>
        </SettingSection>
