...
import { IMSymbolObject } from '../config'
...
export const defaultSymbol = {
  ...
  /* eslint-disable */
  imageData:
    "iVBORw0KGgoAAAANSUhEUgAAADQAAAA0CAYAAADFeBvrAAAABGdBTUEAALGPC/xhBQAAAAlwSFlzAAAOwQAADsEBuJFr7QAAABl0RVh0U29mdHdhcmUAUGFpbnQuT\
...
export default class SymbolSettings extends React.PureComponent<Props, State> {
  private jsonUtils: typeof __esri.jsonUtils = null;
  private updatedSymbol:
    | __esri.SimpleMarkerSymbol
    | __esri.PictureMarkerSymbol
    | __esri.PointSymbol3D;
  constructor(props) {
  ...
  componentDidMount = () => {
    ...
    if (!this.state.apiLoaded) {
      loadArcGISJSAPIModules(["esri/symbols/support/jsonUtils"]).then(
        (modules) => {
          [this.jsonUtils] = modules;
          if (this.props.config && this.props.config.defaultPointSymbol) {
            pointSymbol = this.jsonUtils.fromJSON(
              this.props.config.defaultPointSymbol
            );
          } else {
  ...
  onSymbolClick = () => {
    this.setState(
      ...
      () => {
        this.props.onGeneralSettingsUpdated(
          "defaultPointSymbol",
          this.updatedSymbol?.toJSON()
        );
      }
    );
        ...
        <SettingRow flow={"wrap"}>
          {this.state.defaultSymbol && (
            <SymbolSelector
              ...
              css={getZTopIndex(this.props.theme)}
