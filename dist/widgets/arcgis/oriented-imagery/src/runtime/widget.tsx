    ...
    for (let a = 0; a < frustums.length; a++) {
      if (frustums[a]) { //#625
        if (imageID === frustums[a].imageID) {
          currentCoverageFrustum = new Graphic({ geometry: Mesh.fromJSON(frustums[a]), symbol: this.activeFrustumSymbol, visible: this.coverageFlag.similarCoverage, attributes: { imageID: frustums[a].imageID, id: 'oi-polygons' } })
        } else {
