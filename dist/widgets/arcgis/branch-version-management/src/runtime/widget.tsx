...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig>, any> {
                    ...
                    () => {
                      const foundVersion = this.state.versionList.filter(
                        (vl: any) => {
                          return (
                            vl.versionName ===
                            config.versionConfig.services[0]
                              .configuredSettings.startupVersion ||
                            vl.versionGuid ===
                            config.versionConfig.services[0]
                              .configuredSettings.startupVersion
                          )
  ...
  _populateSwitchableSources = async (status: string) => {
    ...
    if (this.props.useDataSources) {
      if (this.props.useDataSources.length > 0) {
        await ds.createDataSourceByUseDataSource(
  ...
  render () {
    ...
    return (
      ...
      >
        {this.state.isReady && !this.state.showVersionSettingsModal && this.renderServiceSelector()}
        ...
        <div id={this.widRefForPopper}></div>
      </div>
    )
  }
          ...
          <div style={{ flex: 3 }}>
            ...
            >
              <TextArea
                ...
                value={this.state.versionObject.versionName}
                onAcceptValue={(name) => {
                  if (name !== '') {
                        ...
                        if (serviceSwitch) {
                          ...
                          const persist = this.state.versionList.filter(
                            (v: any) => {
                              return (
                                v.versionGuid ===
                                configObj.configuredSettings.persistVersion
                              )
                            }
                          )
                          if (persist.length > 0) {
                            ...
                            const found = this.state.versionList.filter(
                              (v: any) => {
                                return (
                                  v.versionGuid ===
                                  configObj.configuredSettings.startupVersion
                                )
                              }
                            )
                            if (found.length > 0) {
